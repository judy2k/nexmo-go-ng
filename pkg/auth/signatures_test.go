...
func TestGenerateSignature(t *testing.T) {
	...
	generateSignature(params, []byte("abcde"), Md5Hash)
...
func TestGenerateSignatureWithMD5Hash(t *testing.T) {
	...
	sig := generateSignature(params, []byte("abcde"), Md5Hash)
...
func TestGenerateSignatureWithMD5HMAC(t *testing.T) {
	...
	sig := generateSignature(params, []byte("abcde"), Md5Hmac)
...
func TestGenerateSignatureWithSHA1HMAC(t *testing.T) {
	...
	sig := generateSignature(params, []byte("abcde"), Sha1Hmac)
...
func TestGenerateSignatureWithSHA256HMAC(t *testing.T) {
	...
	sig := generateSignature(params, []byte("abcde"), Sha256Hmac)
...
func TestGenerateSignatureWithSHA512HMAC(t *testing.T) {
	...
	sig := generateSignature(params, []byte("abcde"), Sha512Hmac)
...
func TestIfSignWorks(t *testing.T) {
	params := url.Values{}
	params.Set("beta", "space to grow")
	params.Set("alpha", "Fish & Chips")
	params.Set("theta", "a=b+c")
	...
	for _, sm := range signatureMethods {
		Sign(params, []byte("abcde"), sm)
		if !ValidateSignature(params, []byte("abcde"), sm) {
			t.Errorf("ValidateSignature fails on a valid %x signature!", sm)
		}
	}
}
